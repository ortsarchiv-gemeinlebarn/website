...
class GOA_PDF extends PDF_TextBox {
	...
	//________________ Fonction encodage et dessin du GOA_PDF _____________________
	...
	function GOAHeadline($x, $y, $width, $deph, $text){
		...
		$this->SetFont('Josefin','B', $font_size);
		$this->Cell($width, $height, iconv('UTF-8', 'windows-1252', $text));

		return $height + 0.5;
	}

	function GOAPrintKeyValue ($x, $y, $width, $key, $value, $align = 'J'){
		...
		$this->SetFont('Josefin','', 7);
		$this->setXY($x, $y + 1);
		$this->MultiCell($width, 3, iconv('UTF-8', 'windows-1252', $key), $border);

		$this->SetTextColor(0, 0, 0);
		$this->SetFont('Josefin','', 9);
		$this->setXY($x, $y + 4.5);
		$height = 4.5 + $this->GetMultiCellHeight($width, 4, iconv('UTF-8', 'windows-1252', $value));
		$this->MultiCell($width, 4, iconv('UTF-8', 'windows-1252', $value), $border, $align);

		$this->Line($x, $y + $height + 1, $x + $width, $y + $height + 1);
